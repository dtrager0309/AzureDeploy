                ...
                if (csSabre.CrearSesion())
                {
                    try
                    {
                        ...
                        if (bolContinuar)
                        {
                            ...
                            try
                            {
                                if (DataPQ.colPQItinerarios.Count > 0)//(RS.TravelItinerary.ItineraryInfo.ItineraryPricing.PriceQuote.Length > 0)
                                {
                                    if (DataPQ.colPQItinerarios[0].PorComicion != null)//(RS.TravelItinerary.ItineraryInfo.ItineraryPricing.PriceQuote[0].PricedItinerary[0].InputMessage.IndexOf("ÂKP") > -1
                                                                                       //|| RS.TravelItinerary.ItineraryInfo.ItineraryPricing.PriceQuote[0].PricedItinerary[0].InputMessage.IndexOf("¥KP") > -1)
                                    {
                                        decComisionEjecutiva = Convert.ToDecimal(DataPQ.colPQItinerarios[0].PorComicion);//Convert.ToDecimal(RS.TravelItinerary.ItineraryInfo.ItineraryPricing.PriceQuote[0].SignatureLine[0].CommissionAmount);
                                        bolTieneKP = true;
                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Comision de ejecutiva: " + decComisionEjecutiva.ToString("#,##0.00"));
                                    }
                                    else
                                    {
                                        decComisionEjecutiva = 0;
                                        bolTieneKP = false;
                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "No tiene comision de ejecutiva (A)");
                                    }
                                }
                                else
                                {
                                    decComisionEjecutiva = 0;
                                    bolTieneKP = false;
                                    Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "No tiene comision de ejecutiva (B)");
                                }
                            }
                            catch (Exception ex)
                            {
                                Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "No se pudo leer la comision de ejecutiva, err: " + ex.Message);
                                decComisionEjecutiva = 0;
                            }
                            ...
                            {
                                string strTicketDesignator = "";
                                string strTourCode = "";
                                string strEndoso = "";

                                if (strTipoReserva == "PROMOCIONALMYFARES")
                                {
                                    decComisionBase = 0;
                                    decOverAgencia = 0;
                                }
                                ...
                                {
                                    //Si no tiene KP, vamos a buscar la comision base
                                    decComisionBase = csWSSabre.decTraeComisionBase(strReserva,
                                        strLineaAerea,
                                        strClase,
                                        strOrigen);
                                    Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Comision base: " + decComisionBase.ToString("#,##0"));

                                    if (decComisionBase == 0)
                                    {
                                        decComisionBase = 1;
                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Comision base se deja en 1 por defecto");
                                    }

                                    //Validamos que todos los segmentos tengan igual clase, sino dejamos la comision base en cero
                                    //Se elimina validación a por consulta a Soledad Puga, 18/07/2011
                                    //for (int i = 0; i < RS.TravelItinerary.ItineraryInfo.ReservationItems.Length; i++)
                                    //{
                                    //    if (RS.TravelItinerary.ItineraryInfo.ReservationItems[i].Air != null)
                                    //    {
                                    //        if (RS.TravelItinerary.ItineraryInfo.ReservationItems[i].Air.ResBookDesigCode != strClase)
                                    //        {
                                    //            Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Reserva tiene clase distinta en el segmento " + i.ToString() + ", "
                                    //                + "Clase orig:" + strClase + " Clase seg:" + RS.TravelItinerary.ItineraryInfo.ReservationItems[i].Air.ResBookDesigCode
                                    //                + ", se deja la comision base en cero");
                                    //            decComisionBase = 0;
                                    //        }
                                    //    }
                                    //}

                                    //Se agrega validación nacida de la consulta anterior a S.P.
                                    if (strOrigen != "SCL")
                                    {
                                        decComisionBase = 0;
                                    }
                                    ...
                                    //Luego revisamos si tiene Over Agencia
                                    DataSet dsOverAgencia = new DataSet();

                                    decOverAgencia = 0;

                                    dsOverAgencia = csWSSabre.dsTraeOverAgencia(strReserva,
                                            strLineaAerea,
                                            strClase,
                                            strOrigen,
                                            strDestinoComision,
                                    ...
                                    if (dsOverAgencia.Tables.Count > 0)
                                    {
                                        if (dsOverAgencia.Tables[0].Rows.Count > 0)
                                        {
                                            decOverAgencia = Convert.ToDecimal(dsOverAgencia.Tables[0].Rows[0]["EMI_OVER_AGENCIA"].ToString());
                                            strTourCode = dsOverAgencia.Tables[0].Rows[0]["EMI_TOUR_CODE_OVER_AGENCIA"].ToString().Trim();
                                            strEndoso = dsOverAgencia.Tables[0].Rows[0]["EMI_ENDOSO_OVER_AGENCIA"].ToString().Trim();
                                            strTicketDesignator = dsOverAgencia.Tables[0].Rows[0]["EMI_TICKET_DESIGNATOR_OVER_AGENCIA"].ToString().Trim();

                                            Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Over Agencia: " + decOverAgencia.ToString("#,##0"));

                                            //VALIDAMOS LOS DEMAS SEGMENTOS
                                            for (int i = 0; i < RS.TravelItinerary.ItineraryInfo.ReservationItems.Length; i++)
                                            {
                                                if (RS.TravelItinerary.ItineraryInfo.ReservationItems[i].FlightSegment[0] != null)
                                                {
                                                    //Validamos que la clase del segmento no sea distinta a la clase inicial
                                                    //y si es distinta entonces no puede ser distinta a la clase de tarifa combinada
                                                    //if (RS.TravelItinerary.ItineraryInfo.ReservationItems[i].Air.ResBookDesigCode != strClase
                                                    //    && RS.TravelItinerary.ItineraryInfo.ReservationItems[i].Air.ResBookDesigCode.IndexOf(dsOverAgencia.Tables[0].Rows[0]["EMI_TARIFA_COMBINADA_OVER_AGENCIA"].ToString()) == -1)

                                                    if (RS.TravelItinerary.ItineraryInfo.ReservationItems[i].FlightSegment[0].ResBookDesigCode != strClase
                                                        && dsOverAgencia.Tables[0].Rows[0]["EMI_TARIFA_COMBINADA_OVER_AGENCIA"].ToString().IndexOf(RS.TravelItinerary.ItineraryInfo.ReservationItems[i].FlightSegment[0].ResBookDesigCode.ToString()) == -1)
                                                    {
                                                        decOverAgencia = 0;
                                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Se deja over agencia en cero, Clase Orig: " + strClase
                                                            + ", Clase Comb: " + dsOverAgencia.Tables[0].Rows[0]["EMI_TARIFA_COMBINADA_OVER_AGENCIA"].ToString()
                                                            + ", Clase Seg: " + RS.TravelItinerary.ItineraryInfo.ReservationItems[i].FlightSegment[0].ResBookDesigCode);
                                                        break;
                                                    }



                                                    strLineaAereaSegmento = RS.TravelItinerary.ItineraryInfo.ReservationItems[i].FlightSegment[0].MarketingAirline.Code;

                                                    if (strLineaAereaSegmento != strLineaAerea
                                                        && strLineaAereaSegmento != dsOverAgencia.Tables[0].Rows[0]["EMI_CODESHARE_OVER_AGENCIA"].ToString())
                                                    {
                                                        decOverAgencia = 0;
                                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Se deja over agencia en cero, Clase Orig: " + strClase
                                                            + ", Clase Comb: " + dsOverAgencia.Tables[0].Rows[0]["EMI_TARIFA_COMBINADA_OVER_AGENCIA"].ToString()
                                        ...
                                        {
                                            decOverAgencia = 0;
                                            Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "No encontró datos para Over Agencia");
                                        }
                                    }
                                    else
                                    {
                                ...
                                //Sumamos las comisiones
                                decComisionFinal = decComisionBase + decOverAgencia;

                                stbComandoEmision.Append("W¥" + strPQ);
                                //Se comenta validacion (decOverAgencia > 0) en caso de lineas aereas que tienen configurado
                                //over agencia en cero pero que igual incluye TicketDesignator
                                //if (decOverAgencia > 0)
                                //{
                                if (strTicketDesignator != "")
                                {
                                    stbComandoEmision.Append("//DPO-");
                                    stbComandoEmision.Append(strTicketDesignator);
                                }
                                //}
                                stbComandoEmision.Append("¥KP");


                                //Si es promocional dejamos la comision por defecto
                                if (strTipoReserva == "PROMOCIONALMYFARES")
                                {
                                    decComisionFinal = 0;
                                    try
                                    {
                                        decComisionFinal = Convert.ToDecimal(System.Configuration.ConfigurationManager.AppSettings["ComisionNegociadas"]);
                                    }
                                    catch (Exception ex)
                                    {
                                        Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "ERROR LEYENDO COMISION NEGOCIADA: " + ex.Message);
                                    }
                                }

                                //Si es un valor "cerrado" entonces agregamos al comando solo la parte entera
                                if (decComisionFinal - Convert.ToInt32(decComisionFinal) > 0)
                                {
                                    stbComandoEmision.Append(decComisionFinal.ToString().Replace(",", "."));
                                }
                                else
                                {
                                    stbComandoEmision.Append(Convert.ToInt32(decComisionFinal).ToString());
                                }

                                //Si hay over agencia agregamos el tourcode, endoso o ticket designator 
                                //Se comenta validacion (decOverAgencia > 0) en caso de lineas aereas que tienen configurado
                                //over agencia en cero pero que igual incluye TourCode
                                //if (decOverAgencia > 0)
                                //{
                                if (strTourCode != "")
                                {
                                    stbComandoEmision.Append("¥U" + strTourCode);
                                }
                                if (strEndoso != "")
                                {
                                    stbComandoEmision.Append("¥EO/" + strEndoso);
                                }
                                //}
                            }
                    ...
                    finally
                    {
                        ...
                        if (!NoMsgexisteError)
                        {
                            ...
                            Log.grabaLog("wsSabre_bolEmiteReserva", strReserva, "Revalidacion emison ticket en sabre.");
                            RSRespuesta = csSabre.ConsultarPNR_New(strReserva);
